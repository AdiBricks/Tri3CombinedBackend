...
class UserAPI:      
    class _CRUD(Resource):  # User API operation for Create, Read.  THe Update, Delete methods need to be implemeented
        ...
        def get(self, _): # Read Method, the _ indicates current_user is not used
            ...
            hashmap_value = db.session.query(User._hashmap).filter_by(_uid=uid).scalar()
            
            all_users = db.session.query(User).order_by(User._avgGrade.desc()).all()
            grades = [(u.uid, u._avgGrade) for u in all_users]
            ...
            user_grade = db.session.query(User._avgGrade).filter_by(_uid=uid).scalar()
            user_rank = binary_search(grades, user_grade)
